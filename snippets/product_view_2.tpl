<figure>
    ...
    <figcaption class="{if $settings->product_name_align == 'left'}align-left{elseif $settings->product_name_align == 'right'}align-right{else}align-center{/if}">
        ...
        <p>{if $p->on_sale}<span class="old-price">{$p->price_regular|money}</span><span class="new-price">{$p->price_min|money}</span>{else}{$p->price_min|money}{/if}</p>
        </div>
        <input type="hidden" name="id" value="{$p->first_available_variant->id}" data-price="{$p->first_available_variant->price|money_without_currency}" data-property1="{$p->first_available_variant->property1}" data-property2="{$p->first_available_variant->property2}" data-property3="{$p->first_available_variant->property3}" data-product-id="{$p->id}" data-variant-id="{$p->first_available_variant->id}"/>
    </figcaption>
</figure>
